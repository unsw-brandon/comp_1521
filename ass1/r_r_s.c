...
/* -------------------------------- Subset 2 -------------------------------- */
int run_game(char input);
void display_game();
int maybe_print_player(int row, int column);
...
void init_map() {
    ...
    if (i < MAP_HEIGHT) {
        ...
        init_map_loop_2:
        if (j < MAP_WIDTH) {
            g_map[i][j] = EMPTY_CHAR;
            j = j + 1;
            goto init_map_loop_2;
        }
        i = i + 1;
        goto init_map_loop_1;
    }

    // Hard code some things onto the map for easier testing.
    g_map[6][0] = WALL_CHAR;
    g_map[6][1] = TRAIN_CHAR;
    g_map[6][2] = CASH_CHAR;
    g_map[8][2] = BARRIER_CHAR;
}
...
int run_game(char input) {
    ...
    handle_command(g_map, &g_player, &g_block_spawner, input);
    return handle_collision(g_map, &g_player);
}
...
void display_game() {
    ...
    if (i >= 0) {
        ...
        if (j < MAP_WIDTH) {
            ...
            if (!maybe_print_player(i, j)) {
                char map_char = g_map[i][j];
...
int maybe_print_player(int row, int column) {
    if (row == PLAYER_ROW && column == g_player.column) {
        if (g_player.state == PLAYER_RUNNING) {
            printf(PLAYER_RUNNING_SPRITE);
        } else if (g_player.state == PLAYER_CROUCHING) {
            printf(PLAYER_CROUCHING_SPRITE);
        } else if (g_player.state == PLAYER_JUMPING) {
...
void handle_command(char map[MAP_HEIGHT][MAP_WIDTH], struct Player *player,
                    struct BlockSpawner *block_spawner, char input) {
    if (input == LEFT_KEY) {
        if (g_player.column > 0) {
            --g_player.column;
        }

    } else if (input == RIGHT_KEY) {
        if (g_player.column < MAP_WIDTH - 1) {
            ++g_player.column;
        }
        ...
        if (g_player.state == PLAYER_RUNNING) {
            ...
            g_player.action_ticks_left = ACTION_DURATION;
        }

    } else if (input == CROUCH_KEY) {
        // Player must be in normal state (running) to crouch
        if (g_player.state == PLAYER_RUNNING) {
            g_player.state = PLAYER_CROUCHING;
            g_player.action_ticks_left = ACTION_DURATION;
        }
